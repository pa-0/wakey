...
// Function prototypes so that our main loop can be at the top of the file for
...
static volatile uint32_t timer32 = 0;
...
void injkeys(char *source, uint8_t mod) {
  ...
  if (g_usbd_is_connected) {
    ...
    for (int j = 0; j < lstr; j++) {
      ...
      } else if (source[j] >= 'A' && source[j] <= 'Z') { // uppercase
...
int main(void) {
  ...
  while (1) {
    ...
    // variable will overflow and still needs to operate
    ...
    // Go through each of the capsense values to determine when the buttons have
    // been pressed
	uint32_t sum = 0;
    for (int i = 0; i < 4; i++) {
	...
	if (is_pressed != was_pressed) {
		...
		if (is_pressed) {
			// left control
			uint8_t key_down[8] = {0, 0, 0xe0, 0, 0, 0, 0, 0};
			success = 0 != usbd_ep_write_packet(g_usbd_dev, 0x81, key_down, 8);
			gpio_clear(LED_RED_PORT, LED_RED_PIN);
		} else {
			uint8_t key_up[8] = {0, 0, 0, 0, 0, 0, 0, 0};
			success = 0 != usbd_ep_write_packet(g_usbd_dev, 0x81, key_up, 8);
			gpio_set(LED_RED_PORT, LED_RED_PIN);
		}
		if (success) {
...
static void ACMP_CapsenseChannelSet(uint32_t channel) {
  ...
  if (channel == 0) {
    MMIO32(ACMP0_INPUTSEL) =
        (acmpResistor0 << _ACMP_INPUTSEL_CSRESSEL_SHIFT) |
        ACMP_INPUTSEL_CSRESEN | (false << _ACMP_INPUTSEL_LPREF_SHIFT) |
        (0x3f << _ACMP_INPUTSEL_VDDLEVEL_SHIFT) // 0x3f for channel 0 and 0x3d
                                                // for channel 1
    ...
    MMIO32(ACMP0_INPUTSEL) =
        (acmpResistor0 << _ACMP_INPUTSEL_CSRESSEL_SHIFT) |
        ACMP_INPUTSEL_CSRESEN | (false << _ACMP_INPUTSEL_LPREF_SHIFT) |
        (0x3d << _ACMP_INPUTSEL_VDDLEVEL_SHIFT) // 0x3f for channel 0 and 0x3d
                                                // for channel 1
        | ACMP_INPUTSEL_NEGSEL(ACMP_INPUTSEL_NEGSEL_CAPSENSE) |
        (channel << _ACMP_INPUTSEL_POSSEL_SHIFT);
  } else if (channel == 2) {
...
void capsense_stop(void) { g_capsense_running = false; }
...
void setup_acmp_capsense(const struct acmp_capsense_init *init) {
  ...
  MMIO32(ACMP0_INPUTSEL) =
      (init->resistor << _ACMP_INPUTSEL_CSRESSEL_SHIFT) |
...
static void setup_capsense(void) {
  ...
  TIMER1_CTRL = TIMER_CTRL_PRESC(TIMER_CTRL_PRESC_DIV1024) |
                TIMER_CTRL_CLKSEL(TIMER_CTRL_CLKSEL_CC1);
  TIMER1_TOP = 0xFFFF;

  /* Set up TIMER1 CC1 to trigger on PRS channel 0 */
  TIMER1_CC1_CTRL =
      TIMER_CC_CTRL_MODE(TIMER_CC_CTRL_MODE_INPUTCAPTURE) /* Input capture */
  ...
  PRS_CH0_CTRL =
      PRS_CH_CTRL_EDSEL_POSEDGE /* Posedge triggers action */
